...
test("placing a ship out of bound error", () => {
    ...
    expect(() => gameBoard.placeShip(gameBoard.boardLength + 1, 0, 0, Direction.HORIZONTAL)).toThrow("Invalid ship placement: out-of-bounds");
    expect(() => gameBoard.placeShip(2, gameBoard.boardLength - 1, 0, Direction.VERTICAL)).toThrow("Invalid ship placement: out-of-bounds");
    expect(() => gameBoard.placeShip(2, 0, gameBoard.boardLength - 1, Direction.HORIZONTAL)).toThrow("Invalid ship placement: out-of-bounds");
});

test("placing a ship on another ship error", () => {
    const gameBoard = new Gameboard();
    gameBoard.placeShip(5, 0, 2, Direction.VERTICAL);
    expect(() => gameBoard.placeShip(3, 3, 0, Direction.HORIZONTAL)).toThrow("Invalid ship placement: overlapping another ship");
});
...
test("receive attack hit same spot twice error", () => {
    const gameBoard = new Gameboard();
    gameBoard.receiveAttack(0, 0);
    expect(() => gameBoard.receiveAttack(0, 0)).toThrow("This coordinate has already been attacked");
});

test("receive attack hit is out of bound", () => {
    const gameBoard = new Gameboard();
    expect(() => gameBoard.receiveAttack(10, 0)).toThrow("Invalid attack: coordinates out of bounds");
});
