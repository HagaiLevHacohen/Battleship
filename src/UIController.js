...
class UIController {
  ...
  renderBoard(player) {
    if (player.isComputer) {
      ...
      for (let hit of board.successfulHitsList) {
        ...
        const cell = this.computerBoardElement.querySelector(
  ...
  pregameSetHandlers(player, onPlacementComplete) {
    ...
    cells.forEach((cell) => {
      ...
      cell.addEventListener("click", (e) =>
        this.pregameClickHandler(e, player, onPlacementComplete),
      );
  ...
  showPreviewFromIndex(datasetIndex) {
    ...
    for (let k = 0; k < this.placementLength; k++) {
      const r =
        this.placementDirection === Direction.HORIZONTAL ? row : row + k;
      const c =
        this.placementDirection === Direction.HORIZONTAL ? col + k : col;
