...
test("Ship constructor - throws an error if length is 0", ()=> {
    expect(() => new Ship(0)).toThrow('Ship length must be a number greater than 0.');
});

test("Ship constructor - throws an error if length is negative", ()=> {
    expect(() => new Ship(-5)).toThrow('Ship length must be a number greater than 0.');
});

test("Ship constructor - throws an error if length is not a number", ()=> {
    expect(() => new Ship('cat')).toThrow('Ship length must be a number greater than 0.');
});
